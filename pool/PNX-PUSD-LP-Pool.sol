...
// File: contracts/bsd/pool/PNX-PUSD-LP-Pool.sol
...
contract PNX_PUSD_LP_Pool is LPTokenWrapper {
    using SafeMath for uint256;
    using SafeERC20 for IERC20;
    IERC20 public _base;
    IERC20 public _pusd;
    ...
    function getTLV() public view returns (uint) {
        (, uint amountPUSD) = _factory.getReserves(address(_pnx), address(_pusd));
        (uint amountPusd, uint amountUsdt) = _factory.getReserves(address(_pusd), address(_usdt));
        return (_lpToken.balanceOf(address(this)) * amountPUSD * 2 * amountUsdt) / (_lpToken.totalSupply() * amountPusd);
    }
    ...
    function getPersonalAPR(address user) public view returns (uint) {
        ...
        uint b = (balanceOf(user) * amountPUSD * 2 * amountUsdt) / (_lpToken.totalSupply() * amountPusd);
        return a * 100 / b;
    }
